...
const CourseDetail = ({ course }) => {
  ...
  return (
    <div>
      ...
      <Link href='/courses' className="flex items-center gap-2 text-lg font-semibold mb-5 hover:text-[#B12531] transition-colors duration-300 ease-in-out w-fit"><IoMdArrowBack className="text-lg" />Back to Courses</Link>
          <div className="grid lg:grid-cols-3 gap-12">
            {/* Main */}
            <div className="lg:col-span-2 space-y-12">
              {/* Overview */}
              <div>
                <h2 className={`${merriweather.className} text-2xl md:text-3xl font-bold mb-4`}>
                  Course Overview
                </h2>
              ...
              {/* Syllabus */}
              <div>
                <h2 className={`${merriweather.className} text-2xl md:text-3xl font-bold mb-6`}>
                  Course Syllabus
                </h2>
                ...
                <div className="space-y-4">
                  {course.syllabus.map((module, index) => (
                    <div key={index} className="bg-card rounded-xl p-6 shadow">
                      <h3 className={`${merriweather.className} text-lg font-semibold mb-3`}>
                        {module.title}
                      </h3>
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {course.outcomes.map((item, i) => (
                    <div key={i} className="flex items-center gap-2 bg-secondary/50 p-4 rounded-lg">
              ...
              <div>
                ...
                <div className="bg-card p-6 rounded-xl shadow flex gap-6">
                  ...
                  />
                  <div>
                    <h3 className={`${merriweather.className} text-xl font-semibold`}>
              ...
              <div className="sticky top-28 bg-card rounded-2xl shadow overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  <div className="text-center">
                    <p className={`${merriweather.className} text-3xl text-[#B12531]`}>
                      {course.price}
                    </p>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      <span className="flex items-center gap-2 ">
                        <FaClock className="text-[#B12531] text-base" /> Duration
                      </span>
                      <span>{course.duration}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="flex items-center gap-2 text-muted-foreground">
                        <FaBookOpen className="text-[#B12531] text-base"  /> Level
                      </span>
                      <span>{course.level}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="flex items-center gap-2 text-muted-foreground">
                        <FaUsers className="text-[#B12531] text-base"  /> Students
                      </span>
                      <span>{course.students}+</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="flex items-center gap-2 text-muted-foreground">
                        <FaAward className="text-[#B12531] text-base"  /> Certificate
                      </span>
                      <span>Yes</span>
                    </div>
                  </div>
